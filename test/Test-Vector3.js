var Vector3 = require('../src/Vector3'),
...
describe('Vector3', function(){
    ...
    describe('#isEqual', function(){
        it('should be equal', function(){
            assert.equal(new Vector3([1, 2, 3]).isEqualTo(new Vector3([1, 2, 3])), true);
        });
        it('should not be equal', function(){
            assert.equal(new Vector3([1, 2, 3]).isEqualTo(new Vector3([4, 5, 6])), false);
        });
    });
    ...
    describe('#cross', function(){
        it('should return cross product of this vector and the specified vector', function(){
            var vec1 = new Vector3([2, 14, 32]);
            var vec2 = new Vector3([46, 11, 98]);
    ...
    describe('#dot', function(){
        it('should return the dot product of this vector with another vector', function(){
            var vec1 = new Vector3([34, 17, 84]);
            var vec2 = new Vector3([56, 41, 28]);
            assert.equal(vec1.dot(vec2), 4953);
        });
    });
    describe('#isValid', function(){
        it('should be valid', function(){ assert.equal(new Vector3([1, 2, 3]).isValid(), true) });
        it('should not be valid', function(){ assert.equal(new Vector3(["a", "b", "c"]).isValid(), false) });
        it('should not be valid', function(){ assert.equal(new Vector3([NaN, NaN, NaN]).isValid(), false) });
        it('should not be valid', function(){ assert.equal(new Vector3({}).isValid(), false) });
        it('should not be valid', function(){ assert.equal(new Vector3({invalid:true}).isValid(), false) });
    });
    ...
    describe('#normalized', function(){
        it('should return a Vector object with a length of one, pointing in the same direction as this Vector object', function(){
            var vec1 = new Vector3([42, 77, 29]).normalized();
